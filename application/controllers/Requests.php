...
class Requests extends CI_Controller {
    ...
    private function sendMail($id, $transition)
    {
        ...
        $leave = $this->leaves_model->getLeaves($id);
        $submit_date = $leave['row_create_date'];
        $employee = $this->users_model->getUsers($leave['employee']);
        ...
        switch ($transition) {
            case LMS_REQUESTED_ACCEPTED:
                ...
                $approver_model = $this->users_model->getUsers($approver_id);
                $message = $this->parser->parse('emails/' . $employee['language'] . '/request_accepted', $data, TRUE);

                $approver_email = $approver_model["email"];
                $approver_user = str_replace("@summitcommunications.net","",$approver_email);
                
                $super_visor = $supervisor->email;
                $employee_email = $employee['email'];
                $employee_user = str_replace("@summitcommunications.net","",$employee_email);
                // Get info from hr tool db
                $hr_employee_info = file_get_contents("http://172.16.136.35/micro_service_api/public/api/get_user_by_email?email=$employee_email@summitcommunications.net"); 
                $hr_info_json = json_decode($hr_employee_info,true);
                $list = $hr_info_json["user_list"];
                $user_info = $list[0];
                $hr_id = $user_info["hr_id"];
                //$telegram_message = "A leave of $employee  from $startdate to $enddate is approved by $approver_email. This leave request was submitted in $submit_date";
                $telegram_message = $this->format_telegram_msg($employee_user,$hr_id,$startdate,$enddate,$submit_date,$approver_user,$leave['type_name']);
                $result = $this->send_telegram_notification($telegram_message);
                break;
            case LMS_REQUESTED_REJECTED:
...
// sendMailByWrapper($this, $subject, $message, 'raihan.parvez@summitcommunications.net', 'raihan.parvez@summitcommunications.net');     
    }
    /**
     * Format Telegram Message
     * 
     */
    public function format_telegram_msg($emp_email,$hr_id,$start_date,$end_date,$submit_date,$supervisor,$leave_type){
        $msg =  "Leave Approved:"."\n".
                "Employee:".$emp_email."\n".
                "HR ID:".$hr_id."\n".
    ...
    public function send_telegram_notification($msg){
        $telegram_message = urlencode($msg);
        $postdata = http_build_query(
		    array(
				'msg' => $telegram_message,
				'chat_id' => "-1001329204475"
		    )
		);
